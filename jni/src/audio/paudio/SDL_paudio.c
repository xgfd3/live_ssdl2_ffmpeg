...
/* This function waits until it is possible to write a full sound data */
static void
...
#ifdef DEBUG_AUDIO
            fprintf(stderr, "Couldn't get audio data information\n");
#endif
            timeout.tv_sec = 10;
            timeout.tv_usec = 0;
        } else {
...
PAUDIO_OpenDevice(_THIS, const char *devname, int iscapture)
{
    ...
    /*
     * We can't set the data size - just ask the device for the maximum
     * that we can have.
     */
    if (ioctl(fd, AUDIO_BUFFER, &paud_bufinfo) < 0) {
        PAUDIO_CloseDevice(this);
        return SDL_SetError("Couldn't get audio data information");
    }
    ...
    /*
     * We know the data size and the max number of subsequent writes
     *  that can be pending. If more than one can pend, allow the application
     *  to do something like double buffering between our write data and
     *  the device's own data that we are filling with write() anyway.
     *
