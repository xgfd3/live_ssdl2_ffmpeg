...
int BE_CreateWindowFramebuffer(_THIS, SDL_Window * window,
	...
	/* Make sure we have exclusive access to frame data data */
	bwin->LockBuffer();
	...
	*format = BE_BPPToSDLPxFormat(bpp);
	...
	if(bitmap->InitCheck() != B_OK) {
		return SDL_SetError("Could not initialize back data!\n");
	}
	...
	while(bwin->ConnectionEnabled()) {
		if( bwin->Connected() && bwin->BufferExists() && bwin->BufferIsDirty() ) {
			...
			for(i = 0; i < numClips; ++i) {
				...
				/* Copy each row of pixels from the window data into the frame
				   data */
				for(y = 0; y < height; ++y)
				{

					if(bwin->CanTrashWindowBuffer()) {
						goto escape;	/* Break out before the data is killed */
					}
...
void BE_DestroyWindowFramebuffer(_THIS, SDL_Window * window) {
	...
	/* Free and clear the window data */
	BBitmap *bitmap = bwin->GetBitmap();
	delete bitmap;
	bwin->SetBitmap(NULL);
