...
NAS_PlayDevice(_THIS)
{
    while (this->hidden->mixlen > this->hidden->buf_free) {
        /*
         * We think the data is full? Yikes! Ask the server for events,
         *  in the hope that some of them is LowWater events telling us more
         *  of the data is free now than what we think.
         */
        AuEvent ev;
        NAS_AuNextEvent(this->hidden->aud, AuTrue, &ev);
        NAS_AuDispatchEvent(this->hidden->aud, &ev);
...
NAS_OpenDevice(_THIS, const char *devname, int iscapture)
{
    ...
    if (size > 32768)
        size = 32768;    /* So that the data won't get unmanageably big. */
    ...
    AuMakeElementImportClient(elms, this->spec.freq, format,
                              this->spec.channels, AuTrue, size,
                              size / 4, 0, NULL);
