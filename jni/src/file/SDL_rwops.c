...
windows_file_open(SDL_RWops * context, const char *filename, const char *mode)
{
    ...
    context->hidden.windowsio.data.data = NULL;
    context->hidden.windowsio.data.size = 0;
    context->hidden.windowsio.data.left = 0;
    ...
    context->hidden.windowsio.data.data =
        (char *) SDL_malloc(READAHEAD_BUFFER_SIZE);
    if (!context->hidden.windowsio.data.data) {
    ...
    if (h == INVALID_HANDLE_VALUE) {
        SDL_free(context->hidden.windowsio.data.data);
        context->hidden.windowsio.data.data = NULL;
...
windows_file_seek(SDL_RWops * context, Sint64 offset, int whence)
{
    ...
    if (whence == RW_SEEK_CUR && context->hidden.windowsio.data.left) {
        offset -= (long)context->hidden.windowsio.data.left;
    }
    context->hidden.windowsio.data.left = 0;
...
windows_file_read(SDL_RWops * context, void *ptr, size_t size, size_t maxnum)
{
    ...
    if (context->hidden.windowsio.data.left > 0) {
        void *data = (char *) context->hidden.windowsio.data.data +
            context->hidden.windowsio.data.size -
            context->hidden.windowsio.data.left;
        read_ahead =
            SDL_min(total_need, context->hidden.windowsio.data.left);
        SDL_memcpy(ptr, data, read_ahead);
        context->hidden.windowsio.data.left -= read_ahead;
    ...
    if (total_need < READAHEAD_BUFFER_SIZE) {
        if (!ReadFile
            (context->hidden.windowsio.h, context->hidden.windowsio.data.data,
             READAHEAD_BUFFER_SIZE, &byte_read, NULL)) {
            SDL_Error(SDL_EFREAD);
            return 0;
        }
        read_ahead = SDL_min(total_need, (int) byte_read);
        SDL_memcpy(ptr, context->hidden.windowsio.data.data, read_ahead);
        context->hidden.windowsio.data.size = byte_read;
        context->hidden.windowsio.data.left = byte_read - read_ahead;
        total_read += read_ahead;
    } else {
        if (!ReadFile
                 ...
                 size_t num)
{
    ...
    if (context->hidden.windowsio.data.left) {
        SetFilePointer(context->hidden.windowsio.h,
                       -(LONG)context->hidden.windowsio.data.left, NULL,
                       FILE_CURRENT);
        context->hidden.windowsio.data.left = 0;
    }
