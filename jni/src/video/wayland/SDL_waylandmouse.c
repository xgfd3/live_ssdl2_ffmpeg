...
static void
mouse_buffer_release(void *data, struct wl_buffer *data)
{
}
                       ...
                       uint32_t format)
{
    ...
    if (shm_fd < 0)
    {
        fprintf(stderr, "creating mouse cursor data failed!\n");
        return -1;
    }
    ...
    struct wl_shm_pool *shm_pool = wl_shm_create_pool(data->shm,
                                                      shm_fd,
                                                      size);
    d->data = wl_shm_pool_create_buffer(shm_pool,
                                          0,
                                          width,
                                          height,
                                          stride,
                                          format);
    wl_buffer_add_listener(d->data,
...
Wayland_CreateCursor(SDL_Surface *surface, int hot_x, int hot_y)
{
    ...
    if (cursor) {
        ...
        /* Allocate shared memory data for this cursor */
