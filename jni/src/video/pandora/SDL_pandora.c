...
PND_gl_createcontext(_THIS, SDL_Window * window)
{
    ...
    /* Setup color data size */
    if (_this->gl_config.size) {
        wdata->gles_attributes[attr_pos++] = EGL_BUFFER_SIZE;
        wdata->gles_attributes[attr_pos++] = _this->gl_config.size;
    } else {
        wdata->gles_attributes[attr_pos++] = EGL_BUFFER_SIZE;
        wdata->gles_attributes[attr_pos++] = EGL_DONT_CARE;
    }

    /* Setup depth data bits */
    ...
    /* Multisample buffers, OpenGL ES 1.0 spec defines 0 or 1 data */
    ...
    if (configs == 0) {
        ...
        for (it = 0; it < 4; it++) {
            for (jt = 16; jt >= 0; jt--) {
                /* Don't care about color data bits, use what exist */
    ...
    /* Get back stencil and depth data sizes */
    status =
        eglGetConfigAttrib(phdata->egl_display,
                           wdata->gles_configs[wdata->gles_config],
