...
int Android_JNI_OpenAudioDevice(int sampleRate, int is16Bit, int channelCount, int desiredBufferFrames)
{
    ...
    /* Allocating the audio data from the Java side and passing it as the return value for audioInit no longer works on
     * Android >= 4.2 due to a "stale global reference" error. So now we allocate this data directly from this side. */
    ...
    if (audioBuffer == NULL) {
        __android_log_print(ANDROID_LOG_WARN, "SDL", "SDL audio: could not allocate an audio data!");
        return 0;
    }
...
void Android_JNI_WriteAudioBuffer()
{
    ...
    /* JNI_COMMIT means the changes are committed to the VM but the data remains pinned */
}
