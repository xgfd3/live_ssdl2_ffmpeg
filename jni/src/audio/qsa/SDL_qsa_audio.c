...
QSA_WaitDevice(_THIS)
{
    ...
    do {
        ...
        case 0:
            {
                SDL_SetError("QSA: timeout on data waiting occured");
...
QSA_PlayDevice(_THIS)
{
    ...
    /* Write the audio data, checking for EAGAIN (data full) and underrun */
    do {
        written =
        ...
        if (written != towrite) {
            ...
            if ((errno == EAGAIN) && (written == 0)) {
                if (this->hidden->timeout_on_wait != 0) {
                    SDL_SetError("QSA: data playback timeout");
                    return;
                }
            }
...
QSA_OpenDevice(_THIS, const char *devname, int iscapture)
{
    ...
    /*
     * Allocate memory to the audio data and initialize with silence
     *  (Note that data size must be a multiple of fragment size, so find
     *  closest multiple)
     */
    this->hidden->pcm_buf =
