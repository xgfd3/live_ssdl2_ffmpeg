...
DSOUND_WaitDevice(_THIS)
{
    ...
    /* Semi-busy wait, since we have no way of getting play notification
       on a primary mixing data located in hardware (DirectX 5.0)
     */
    ...
    while ((cursor / this->hidden->mixlen) == this->hidden->lastchunk) {
        ...
        /* Try to restore a lost sound data */
...
{
    /* Unlock the data, allowing it to play */
...
/* This function tries to create a secondary audio data, and returns the
   number of audio chunks available in the created data.
*/
static int
CreateSecondary(_THIS, HWND focus)
{
    ...
    /* Try to create the secondary data */
    SDL_zero(format);
    ...
    if ((format.dwBufferBytes < DSBSIZE_MIN) ||
        ...
        return SDL_SetError("Sound data size must be between %d and %d",
    ...
    /* Silence the initial audio data */
...
DSOUND_OpenDevice(_THIS, const char *devname, int iscapture)
{
    ...
    /* The data will auto-start playing in DSOUND_WaitDevice() */
