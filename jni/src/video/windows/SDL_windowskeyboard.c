...
IME_GetReadingString(SDL_VideoData *videodata, HWND hwnd)
{
    ...
    if (videodata->GetReadingString) {
        ...
        if (len) {
            if (len > SDL_arraysize(data))
                len = SDL_arraysize(data);
...
{
    char *s = 0;
    WCHAR data[SDL_TEXTEDITINGEVENT_TEXT_SIZE];
    const size_t size = SDL_arraysize(data);
    data[0] = 0;
    if (videodata->ime_readingstring[0]) {
        size_t len = SDL_min(SDL_wcslen(videodata->ime_composition), (size_t)videodata->ime_cursor);
        SDL_wcslcpy(data, videodata->ime_composition, len + 1);
        SDL_wcslcat(data, videodata->ime_readingstring, size);
        SDL_wcslcat(data, &videodata->ime_composition[len], size);
    }
    else {
        SDL_wcslcpy(data, videodata->ime_composition, size);
    }
    s = WIN_StringToUTF8(data);
