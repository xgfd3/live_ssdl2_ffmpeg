...
/* Allow access to a raw mixing data */
...
/* Streaming functions (for when the input and output data sizes are different) */
...
/* Initialize the stream by allocating the data and setting the read/write heads to the beginning */
#if 0
static int
SDL_StreamInit(SDL_AudioStreamer * stream, int max_len, Uint8 silence)
{
    /* First try to allocate the data */
    stream->data = (Uint8 *) SDL_malloc(max_len);
    if (stream->data == NULL) {
        return -1;
    }
    ...
    /* Zero out the data */
    SDL_memset(stream->data, silence, max_len);

    return 0;
}
#endif

/* Deinitialize the stream simply by freeing the data */
static void
...
SDL_RunAudio(void *devicep)
{
    ...
    Uint32 delay;
    /* For streaming when the data sizes don't match up */
    Uint8 *istream;
    int istream_len = 0;
 ...
 * a device data both here and below in the stream output.
 */
            ...
            if (SDL_StreamLength(&device->streamer) >= stream_len) {
                ...
                /* Ready current data for play and change current data */
                if (stream != device->fake_stream) {
                    current_audio.impl.PlayDevice(device);
                    /* Wait for an audio data to become available */
        ...
        while (device->enabled) {
            ...
            /* Fill the current data with sound */
            if (device->convert.needed) {
                if (device->convert.buf) {
                    stream = device->convert.buf;
                } else {
                  ...
                  int allowed_changes, int min_id)
{
    ...
    /* Allocate a fake audio memory data */
    ...
    /* If the audio driver changes the data size, accept it.
       This needs to be done after the format is modified above,
       otherwise it might not have the correct data size.
     */
