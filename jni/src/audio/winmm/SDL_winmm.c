          ...
          DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
    ...
    /* Only service "data is filled" messages */
    if (uMsg != WIM_DATA)
        return;

    /* Signal that we have a new data of data */
          ...
          DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
    ...
    /* Only service "data done playing" messages */
    if (uMsg != WOM_DONE)
        return;

    /* Signal that we are done playing a data */
    ReleaseSemaphore(this->hidden->audio_sem, 1, NULL);
}
...
WINMM_OpenDevice(_THIS, const char *devname, int iscapture)
{
    ...
    /* Check the data size -- minimum of 1/4 second (word aligned) */
    ...
    /* Create the audio data semaphore */
